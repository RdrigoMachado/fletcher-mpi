...
for tamanho in 56 180 344; do
    for i in 1 2 3; do
        ...
        for recebe in 2 4; do
            tempo=$(mpirun --rankfile 1_maquina_receivers  --use-hwthread-cpus -np 3 ./multiple_files.OpenMP.x TTI $tamanho $tamanho $tamanho 16 12.5 12.5 12.5 0.000685 0.07 $envio $recebe)
...
output_file="many_draco2_16_threads.csv"
echo -n "" > "$output_file"
...
for tamanho in 56 180 344; do
    for i in 1 2 3; do
        ...
        for recebe in 2 4; do
          versao="${send}_packing"
            tempo=$(mpirun --hostfile hostfile --rankfile 2_maquinas_receivers  --use-hwthread-cpus -np 3 ./multiple_files.OpenMP.x TTI $tamanho $tamanho $tamanho 16 12.5 12.5 12.5 0.000685 0.07 $envio $recebe)
          tempo_formatado=$(replace_dot_with_comma)
          linha="$linha;$tempo_formatado"
        done
        done
...
output_file="many_draco2_draco3_16_threads.csv"
echo -n "" > "$output_file"
